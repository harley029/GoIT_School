...
async def get_contact_by_email(contact_email: str, db: AsyncSession = Depends(get_db)):
    contact = await repositories_contacts.get_contact_by_email(contact_email, db)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found"
        )
...
async def get_contact_by_last_name(
    contact_last_name: str, db: AsyncSession = Depends(get_db)
):
    contacts = await repositories_contacts.get_contact_by_last_name(
        contact_last_name, db
    )
    return contacts
...
async def get_contact_by_birthday(
    contact_birthday: str, db: AsyncSession = Depends(get_db)
):
    contacts = await repositories_contacts.get_contact_by_birthday(contact_birthday, db)
    return contacts
