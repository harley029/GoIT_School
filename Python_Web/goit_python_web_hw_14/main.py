...
# REDIS_URL = "redis://localhost:6379"  # для локального запуска (см файл db.py)
REDIS_URL = settings.redis_url # для запуска из Докера (см файл db.py)
@asynccontextmanager
async def lifespan(app: FastAPI):
    ...
    """
    redis_client = await redis.from_url(f"{REDIS_URL}")
